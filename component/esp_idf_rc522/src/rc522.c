...
//            err = rc522_write(rc522, test_addr, test_val);
//
//            if(err == ESP_OK) {
//                err = rc522_read(rc522, test_addr, &tmp);
//
//                if(err == ESP_OK && tmp == test_val) {
//                    pass = 1;
//                }
//            }
//
...
//        }
        ...
        ESP_ERR_RET_GUARD(rc522_write(rc522, RC522_TXMODE_REG, 0x00));
        ESP_ERR_RET_GUARD(rc522_write(rc522, RC522_RXMODE_REG, 0x00));
        ESP_ERR_RET_GUARD(rc522_write(rc522, RC522_MOD_WIDTH_REG, 0x26));

        ESP_ERR_RET_GUARD(rc522_write(rc522, RC522_COMMAND_REG, 0x0F));
        ESP_ERR_RET_GUARD(rc522_write(rc522, RC522_TIMER_MODE_REG, 0x80));//TODO 0x8D
        ESP_ERR_RET_GUARD(rc522_write(rc522, RC522_TIMER_PRESCALER_REG, 0xA9));//3E
        ESP_ERR_RET_GUARD(rc522_write(rc522, RC522_TIMER_RELOAD_LSB_REG, 0xE8));//1E
        ESP_ERR_RET_GUARD(rc522_write(rc522, RC522_TIMER_RELOAD_MSB_REG, 0x03));//00
