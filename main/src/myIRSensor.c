...
/*---------------------------------------------------------------
        ADC General values
---------------------------------------------------------------*/
#define IR_ADC1_CHAN0               ADC_CHANNEL_7
#define EXAMPLE_ADC_ATTEN           ADC_ATTEN_DB_12 //For range to 3.1V
...
//
    ...
    bool processData(int measuredVoltage);
...
bool irSensorMain() {
    ...
    ESP_ERROR_CHECK(adc_oneshot_config_channel(adc1_handle, IR_ADC1_CHAN0, &config));

    //-------------ADC1 Calibration Init---------------//
    adc_cali_handle_t adc1_cali_chan0_handle = NULL;
    bool do_calibration_IR = example_adc_calibration_init(ADC_UNIT_1, IR_ADC1_CHAN0, EXAMPLE_ADC_ATTEN,
    ...
    //-------------ADC1 Main Read Loop---------------//
    while (1) {
        ESP_ERROR_CHECK(adc_oneshot_read(adc1_handle, IR_ADC1_CHAN0, &adc_raw[0]));
        //ESP_LOGI(TAG, "ADC%d Channel[%d] Raw Data: %d", ADC_UNIT_1 + 1, IR_ADC1_CHAN0, adc_raw[0]);

        if (do_calibration_IR) {
            ESP_ERROR_CHECK(adc_cali_raw_to_voltage(adc1_cali_chan0_handle, adc_raw[0], &voltage[0]));
            //ESP_LOGI(TAG, "ADC%d Channel[%d] Cali Voltage: %d mV", ADC_UNIT_1 + 1, IR_ADC1_CHAN0, voltage[0]);
        }

        retProcess = processData(voltage[0]);
...
bool processData(int measuredVoltage) {
    ...
    int range = convertToRange(measuredVoltage);
    ...
    ESP_LOGI(TAG, "Value: %d\t Range: %d\t Count = %d\t currentLevel = %d\t", measuredVoltage, range, counter, currentLevel);
    return false;
}
